export function Carts ({ title, value, percentage, icon, colorVariant }) {
...
return (
<div className={`${variants[colorVariant]} p-5 rounded-[2rem] border relative overflow-hidden flex flex-col justify-between min-h-[150px] shadow-sm`}>
      <div className="flex justify-between items-start">
        <div>
          <p className={`text-[10px] md:text-xs font-bold uppercase leading-tight ${textColors[colorVariant]}`}>
            {title}
</p>
        ...
        <span className={`bg-white/50 rounded-xl text-lg border-2 shadow-sm ${iconPadding} ${iconStyles[colorVariant]}`}>
          {icon}
        </span>
</div>
</div>
